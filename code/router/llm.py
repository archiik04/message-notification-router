...
Costs are asymmetric. Missing something the user genuinely needed (a same-day deadline, a live delivery, a direct ask from someone they engage with) is worse than one extra notification. Do not downgrade a time-bound or transactional message merely because it feels routine.
...
class LLMArbiter:
    ...
    def _facts(
        self, message: Message, content: ContentView, safety: SafetyVerdict, bundle: SignalBundle
    ) -> str:
        """Pre-computed signals, kept separate from the untrusted payload.

        Thresholds are read from live configuration rather than written into the
        text. Hardcoding them once left the model comparing a score against
        stale bounds, which made every borderline row look sub-threshold and
        produced a systematic bias toward digest.
        """
        ...
        lines = [
            ...
            f"engine_priority: {bundle.priority:.2f} "
            f"(notify floor {th.notify_floor:.2f}, mute ceiling {th.mute_ceiling:.2f})",
            ...
            f"concerns_a_live_transaction: {bundle.transactional}",
